...
### P0 compile-signature verification

Verified `src/resolver.rs::manifest_evaluation_plan` has a single `hw: &HardwareProfile` parameter:
...
A static check was run in the packaging sandbox to assert the signature contains exactly one `hw: &HardwareProfile` binding. The sandbox still lacks Rust toolchain binaries, so `cargo check` must run on the handoff machine.
